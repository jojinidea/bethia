...
class Peer(object):
    ...
    def findFileLocation(self, fileID, requestingPeer, fileLocation, predecessorID):
        ...
        peerID = self._ID
        if successorID < peerID: # if successorID < peerID this means we are about to come to the start of the circle. Add 255 to the successorID
            successorID = successorID + 255
        if predecessorID > peerID: # if predecessorID > peerID this means we are at the start of the circle. Add 255 to peerID
            peerID = peerID + 255
        if fileLocation - peerID > abs(fileLocation - successorID):
        ...
        elif fileLocation - peerID <= abs(fileLocation - successorID): # we are the closest peer
    ...
    # 3 requests 1 
    # 3 should check its immediate predecessor & successor
    # if abs distance between immediatesuccessor[0] & file location is smaller than abs distance between immediatepredecessor[0] and file location:
        # abs(4-1) = 3, abs(3-1) = 2, abs(2-1) = 1 (if one of these that is not us is the smallest, keep going)
        # pass to our successor
    # when should we stop
        # abs(2-1) = 1 abs(1-1) = 0, abs(4-1) = 3
    ...
    def sendFile(self, requestingPeer, fileID):
        ...
        while (data):
            ...
            msg = self.encapsulateMsg(data, ACKNum, seqNum)
            if (self._UDPSock.sendto(msg, addr)):
                print("Sending")
                seqNum = seqNum + len(data) # new sequence number is old sequence number plus length of the data that has been sent
                data = f.read(self._MSS) # advance buffer (REMOVE)
                #ready = select.select([self._UDPSock],[],[],timeout)
                # try:
                #   if ready[0]: # AND MESSAGE IS RIGHT TYPE - ACK MSG  
                #        msg, clientAddress = s.recvfrom(4096) 
                #        ACKnum = [int(s) for s in msg.split() if s.isdigit()][0]
                #        if ACKnum == seqNum: # check value of ACK against seq Num of next packet
                #            data = f.read(self._MSS) # advance buffer
    ...
    def encapsulateMsg(self, data, ACKNum, seqNum):
        header = "***HEADER*** ACK_NUM: " + str(ACKNum) + " SEQ_NUM: " + str(seqNum) + " MSS: " + str(self._MSS) + " START"
        msg = header.encode('utf-8') + data # append header to msg
        return msg
  
    def receiveFile(self, msg, addr, openFile): # need to identify that a file transfer is happening
        if openFile == False:
            f = open(received_file.pdf, "wb") # requesting peer opens file
        elif openFile == True:
            msgStart = self.findPayload(msg)
            MSS = self.findMSS(msg)
            f.write(msg[msgStart:msgStart+MSS]) # NEED TO ASK IF WE CAN ASSUME MSS OF ALL PEERS THE SAME
            seqNumber = [int(n) for n in data.split() if n.isdigit()][1] # extracts first number from header
            ACK = len(msg) - msgStart + seqNumber # ACK is the length of the data (not including header) + sequence num
            ACKmsg = "ACK: " + str(ACK)
