...
class Peer(object):
    ...
    def UDPlisten(self):
        ...
        while True:
            ...
            if (time.time() - lastPing > 3):
                if succ0Dead == False:
                    self.sendPingRequest(self._immediateSuccessors[0][0][0],seqNum)
                if succ1Dead == False:
                    self.sendPingRequest(self._immediateSuccessors[1][0][0], seqNum)
                seqNum = seqNum + 1
                lastPingSent = time.time()
            succ0MissedACKs = seqNum - succ0LastACK
            succ1MissedACKs = seqNum - succ1LastACK
            if succ0Dead == False and succ0MissedACKs >= 10: 
                print("Peer %d is no longer alive"%(self._immediateSuccessors[0][0][0]))
                succ0Dead = True
                succ0LastACK = seqNum
                # send TCP query asking for successors
            if succ1Dead == False and succ1MissedACKs >= 10:
                print("Peer %d is no longer alive"%(self._immediateSuccessors[1][0][0]))
                succ1Dead = True
                succ1LastACK = seqNum
                # send TCP query asking for successors
            try: 
                msg, clientAddress= self._UDPSock.recvfrom(4096)
                clientID = int(clientAddress[1])-50000
                if self.identifyMsg(msg) == "PING_RESPONSE":
                    ACKNum = [int(n) for n in msg.split() if n.isdigit()][0]
                    if clientID == self._immediateSuccessors[0][0][0]:
                        succ0LastACK = ACKNum
                    if clientID == self._immediateSuccessors[1][0][0]:
                        succ1LastACK = ACKNum
                    print("A peer response message was received from Peer %d"%(clientID))
                if self.identifyMsg(msg) == "PING_REQUEST":
                    self.sendPingResponse(clientAddress, msg) # sending response back to peer
                ...
                if self.identifyMsg(msg) == "START_FILE_TRANSFER":
                    ...
                    self.receiveFile()
                #print("Self._immediateSuccessors in UDP Listen are", self._immediateSuccessors)
    ...
    def sendPingResponse(self, clientAddress, msg):
        pingResponse = 'PING_RESPONSE '
        seqNum = [int(n) for n in msg.split() if n.isdigit()][0]
        pingResponse = pingResponse + str(seqNum)
    ...
    def sendPingRequest(self, successorID, seqNum):
            #print("sending ping")
            sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM); #UDP
            time.sleep(5)
            message = "PING_REQUEST " + str(seqNum) # add sequence number
            try:
                sock.sendto(message.encode(), ('localhost', int(successorID)))
            except KeyboardInterrupt:
                print("Keyboard interrupt, stopping main loop", file = sys.stderr)
                sys.exit(1)
        seqNum = seqNum + 1
    ...
    def forwardMessage(self, fileLocation):
        ...
        if immediatePredecessor <= peerID and immediateSuccessor >= peerID: # we are not at start of the DHT
            ...
            distances = [abs(immediatePredecessor-fileLocation), abs(peerID-fileLocation), abs(immediateSuccessor-fileLocation)]
            print("distances in else is", distances)
            ...
            if self.closestPeer(distances) or self.closestPeer(distancesModified):
                print("Returning false, we have the file")
                return False
            print("Returning true, need to forward")
            return True
    """
