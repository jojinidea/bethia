...
class Peer(object):
    def __init__(self, ID, MSS, dropProb):
        self._ID = int(ID)
        self._MSS = int(MSS) 
        self._port = 50000 + int(ID) # NEED TO CHANGE BACK TO 50000
        self._IP = 'localhost'
    ...
    def bindSock(self):
        ...
        self._fileReceiverSock.bind((self._IP, int(self._port)+500)) # we have a UDP file receiver sock that listens to portNum + 500
    ...
    # threaded function continually listening on a peer's fileReceiverSock for a START_FILE_TRANSFER message (at which point it invokes the receiveFile function) 
    ...
    def receiveFile(self, fileID): 
            ...
            while(transferOngoing):
                i = 0
                data, addr = receivingSock.recvfrom(4096) 
