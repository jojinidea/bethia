...
# NEED TO DEAL WITH DUPLICATES (DUPLICATE PACKETS IN CASE ACK DROPPED)
    ...
    def UDPlisten(self):
        while True:
            try: 
                ...
                print("response in UDP listen is", msg)
                ...
                if self.identifyMsg(msg) == "ACK":
                    print("ACK IN UDP LISTEN")
    ...
    def sendFile(self, requestingPeer, fileID): 
        ...
        while (data):
            if (random.uniform(0,1) > self._dropProb): # we have not dropped the packet, so send it and write the corresponding logfile entry
                ...
                while wait == True:
                    try: 
                        response, clientAddress = self._UDPSock.recvfrom(4096)
                        print("Response in file transfer is", response) 
                        if self.identifyMsg(response) == "ACK":
