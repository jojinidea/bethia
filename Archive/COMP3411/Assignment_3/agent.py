...
class Agent(object):
    ...
    def minimax(self, node, depth, maximizingPlayer, game, lastMove):
        ...
        print("At startDepth is %d, lastMove is %d"%(depth, lastMove))
        if self.has_game_ended(node) == True: # if the game has ended on the subboard node, evaluate node as a terminal node
            if self.win_or_lose(node, 0) == 100: # if we have won
                return 100
            elif self.win_or_lose(node, 1) == -100: # if opponent has won
                return -100
            else:                                   # drawn
                return 0
        if depth == 2: 
            ...
            if maximizingPlayer:
                player = 0
                print(node)
                print("In depth == 2, Depth is %d, last move is %d"%(depth, lastMove))
                return self.evaluate_heuristic(game.board[lastMove], player)
            else:
                player = 1
                print(node)
                print("in depth == 2, Depth is %d, last move is %d"%(depth, lastMove))
        ...
        if maximizingPlayer: # if our turn
            ...
            for move in self.legal_moves(node):
                ...
                #game.display_board()
                v = self.minimax(game.board[move], depth+1, False, game, move) # give subboard that we made a move in
            ...
            bestValue = math.inf
            #print("Legal moves are", self.legal_moves(node))
            #print("Past move was %d", lastMove)
            print(self.legal_moves(node))
            for move in self.legal_moves(node):
                ...
                v = self.minimax(game.board[move], depth+1, True, game, move)
                #game.display_board()
