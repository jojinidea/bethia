...
int connected(Graph g, Vertex a, Vertex b);
...
// vs is the array that we want to populate with the vertices that are reachable from v
int within(Graph g, Vertex s, int d, Vertex *vs)    
{

    Queue new = newQueue();
    QueueJoin(new, s); 
    int count = 0; // stores count of number of dequeued items
    int visited[g->nV]; // visited array
    int i = 0;
    int index = 0;
    Item item;
    ...
    while (QueueIsEmpty(new) == 0 && count < d) {
        item = QueueLeave(new);
        count++;
        // for all adjacent neighbours of that item that have not been visited
        for (i = 0; i < g->nV; i++) {
            if (connected(g, item, i) == 1 && visited[i] == 0 && i != s) {
                visited[i] = 1;
                vs[index] = i;
                index++;
                QueueJoin(new, i);  
    ...
    // use a queue
    // much easier
    // enqueue all neighbours of a vertex if we have not exceeded the distance
    // stop when we have dequeued d elements
