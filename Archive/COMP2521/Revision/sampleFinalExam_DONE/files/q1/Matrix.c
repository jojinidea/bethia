...
Matrix multiply(Matrix x, Matrix y)
{
    assert(x != NULL && x->val != NULL);
    assert(y != NULL && y->val != NULL);
    // matrix multiplication is defined if the first matrix's columns == second matrix's rows
    ...
    if (x->ncols == y->nrows) {
        ...
        // loop over all rows of first matrix
        for (row = 0; row < x->nrows; row++) {
            for (col = 0; col < y->ncols; col++) {
                for (index = 0; index < y->ncols; index++) {
                    product = (x->val[row][index] * y->val[index][col]) + product; 
                    printf("Multipling x %d and y %d to product %d\n", x->val[row][index], y->val[index][col], product);
                }
            printf("Row is %d, col is %d\n", row, col);
            new->val[row][col] = product; 
            product = 0;
            }
            col = 0;
            index = 0;
   ...
   // check that matrix x has same number of columns as matrix y's rows
