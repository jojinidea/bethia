...
int isSubtreeLesser(Tree t, int value);
int isSubtreeGreater(Tree t, int value);
void infix(Tree t, int *k, int *count, int **v);
...
int isBST(Tree t) {
    ...
    // if t == NULL
    // return 1
    // if isBST(t->left) && isBST(t->right) && isSubtreeLesser(t->left, t->data) && isSubtreeGreater(t->right, t->data) 
    // return 1
    // return 0
    if (t == NULL) {
        return 1;
    }   
    if (isSubtreeLesser(t->left, t->val) && isSubtreeGreater(t->right, t->val) && isBST(t->left) && isBST(t->right)) {
        return 1;
    }
...
int isSubtreeLesser(Tree t, int data) {
    if (t == NULL) {
        return 1;
    }
    if (t->val <= data && isSubtreeLesser(t->left, data) && isSubtreeLesser(t->right, data)) {
        return 1;
    }
...
int isSubtreeGreater(Tree t, int data) {
    if (t == NULL) {
        return 1;
    }
    if (t->val >= data && isSubtreeGreater(t->left, data) && isSubtreeGreater(t->right, data)) {
        return 1;
    }
return 0;
}
...
Tree successor(Tree t, Tree target) {
    ...
    if (target->right != NULL) {
        ...
        return prev;
        // we have to traverse the tree until we find a node that is before our node
...
void getKthSmallest(Tree t, int* k, int** v) {
    // want to do LNR in order traversal a certain amount of times
    //int c = 0;
    int c = 0;
    int *count = &c;
    infix(t, k, count, v);
...
void infix(Tree t, int *k, int *count, int **v) {
    ...
    printf("Count is %d, k is %d\n", *count, *k);
    infix(t->left, k, count, v);
    if (*k == *count) {
        printf("TEST2\n");
        printf("t->value is %d\n", t->val);
        *v =  &(t->val);
