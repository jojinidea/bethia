...
int validDLList(DLList L)
{
	...
	if (count != L->nitems) {
		fprintf(stderr, "Forward count mismatch; counted=%d, nitems=%d\n",
		        count, L->nitems);
		fprintf(stderr, "L->curr->next->value is %s\n", L->curr->next->value);
		return 0;
	}
...
void DLListBefore(DLList L, char *it)
{
	...
	} else if (L->nitems == 1) {                                             // Case 1: Change first and last pointers
        new->next = L->curr; 
        L->curr->prev = L->first = new;
        L->last = L->curr;
        L->curr = new;
...
void DLListAfter(DLList L, char *it)
{
	...
	} else if (L->nitems == 1) {                                             // Case 1: Need to modify first and last pointers
        L->curr->next = new;
        new->prev = L->curr;
        L->last = new;
        L->first = L->curr;
        L->curr = new; 
    } else if (L->last == L->curr) {                                        // Case 2: Need to modify last pointer
        L->curr->next = new;
        new->prev = L->curr;
        L->last = new;
        L->curr = new;
    } else {  
        DLListNode *temp = L->curr->next;                                   // Case 3: Don't need to modify first/last, insert somewhere in the middle
        L->curr->next = new;
        new->prev = L->curr;
        temp->prev = new;
        new->next = temp;
...
void DLListDelete(DLList L)
{
    ...
    if (L->curr == L->first) {                                              // Case 1: Need to modify first (sub-case: if the list has 1 element)
        if (L->nitems == 1) {
            free(temp->value);
            free(temp);
            L->curr = NULL;
    ...
    } else if (L->curr == L->last) {                                        // Case 2: Need to modify last (sub-case: if the list has 1 element)
        if (L->nitems == 1) {
            free(temp->value);
            free(temp);
            L->curr = NULL;
    ...
    } else {                                                                // Case 3: Deleting an element with at least 1 element to the left & right
