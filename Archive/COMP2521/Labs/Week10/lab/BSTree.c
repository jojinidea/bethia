...
void BSTreeLevelOrder(BSTree t)
{
    ...
    while (QueueIsEmpty(nodes) != TRUE) {
        Item node = QueueLeave(nodes);
        printf("%d ", node->value);
        if (node->left != NULL && node != NULL) {
            QueueJoin(nodes, node->left);
        }
        if (node->right != NULL && node!= NULL) {
            QueueJoin(nodes, node->right);
        }
...
// if t->left == NULL && t->right == NULL, this means we have a leaf, return 1
    if (t == NULL) {
        return 0;
    }
    if (t->left == NULL && t->right == NULL) {
        ...
        int BSTreeNumLeavesL = BSTreeNumLeaves(t->left);
        int BSTreeNumLeavesR = BSTreeNumLeaves(t->right); 
        return BSTreeNumLeavesL + BSTreeNumLeavesR;
    }
