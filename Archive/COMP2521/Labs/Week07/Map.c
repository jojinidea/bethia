...
void testLandSeaLandConnection();
void testSeaConnection();
...
int connections(Map g, LocationID start, LocationID end, TransportID type[])
{
    ...
    if (g->connections[start] != NULL) {
        currNode = g->connections[start]; 
        while (currNode != NULL) {
            if (currNode->v == end) {
                type[numConnections] = currNode->type;
                numConnections++;
            } else if (idToType(start) == LAND && idToType(end) == LAND && idToType(currNode->v) == SEA) {
                temp = currNode;
                if (g->connections[currNode->v] != NULL) {
                    temp = g->connections[currNode->v]; 
                    while (temp != NULL) {
                        if (temp->v == end) {
                            type[numConnections] = BOAT;
                            numConnections++;
                        }
                    temp = temp->next; 
                    }
...
void testNoConnections() {
    ...
    Map europe = newMap();
    assert(connections(europe, nameToID("Le Havre"), nameToID("Geneva"), t) == 0);
    assert(connections(europe, nameToID("Budapest"), nameToID("Castle Dracula"), t) == 0);
    fprintf(stderr, "Assertions for testNoConnections() passed\n");
}

void testOneConnection() {
    TransportID t[MAX_TRANSPORT];
    Map europe = newMap();
    assert(connections(europe, nameToID("Bucharest"), nameToID("Klausenburg"), t) == 1);
    assert(t[0] == ROAD);
    fprintf(stderr, "Assertions for testOneConnection() passed\n");
...
void testLandSeaLandConnection() {
    TransportID t[MAX_TRANSPORT];
    Map europe = newMap();
    int num = connections(europe, nameToID("Rome"), nameToID("Naples"), t);
    assert(num == 3);
    assert(t[0] == BOAT);
    fprintf(stderr, "Assertions for testLandSeaLandConnections() passed\n");
}
...
void testSeaConnection() {
    TransportID t[MAX_TRANSPORT];
    Map europe = newMap();
    int num = connections(europe, nameToID("English Channel"), nameToID("North Sea"), t);
    assert(num == 1);
    assert(t[0] == BOAT);
    fprintf(stderr, "Assertions for testSeaConnection() passed\n");
...
void testMultipleConnections() {
    TransportID t[MAX_TRANSPORT];
    Map europe = newMap();
    int num = connections(europe, nameToID("Alicante"), nameToID("Barcelona"), t);
    assert(num == 2);
    assert(t[0] == BOAT);
    assert(t[1] == RAIL);
    fprintf(stderr, "Assertions for testMultipleConnections() passed\n");
