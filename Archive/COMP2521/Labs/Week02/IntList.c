...
{   
    ...
    if (IsListEmpty(L) == TRUE) {
        InsertFirstElementIntList(L, v);
    } else {
        int Position = FindElementPosition(L, v);
        if (Position == 0) {
            InsertFirst(L, v);
        } else if (Position == L->size) {
            InsertLast(L,v);
        } else {
            InsertElement(L, v, Position);
        }
...
// returns the element's position. 0 means start of the list, 1 means after the first element, etc. 
int FindElementPosition(IntList L, int v) {
    struct IntListNode *curr = L->first;
    int Position = 0;
    int PositionFound = FALSE;
    
    while (PositionFound == FALSE && curr != NULL) {
        if (v <= curr->data) {
...
// inserts first element in an empty list
void InsertFirstElementIntList(IntList L, int v) {
    L->first = newIntListNode(v); 
    L->last = L->first;

}

// places an element in the first position of a non-empty list
void InsertFirst(IntList L, int v) {
...
// function that places a certain element in the nth position of non-empty list (where n = Position)
void InsertElement(IntList L, int v, int Position) {
    struct IntListNode *new = newIntListNode(v); 
    struct IntListNode *curr = L->first; 
...
// function that inserts the last element in a non-empty list
void InsertLast(IntList L, int v) {
    struct IntListNode *new = newIntListNode(v);
    struct IntListNode *tail = L->last; 
