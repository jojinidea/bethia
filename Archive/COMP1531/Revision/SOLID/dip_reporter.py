...
class Reporter(object):
    ...
    def __init__(self, formatter): #injection through constructor
        self._formatter = formatter
    
    def email_report_hours(self,email, time_period,emp_id):
        report_data = ReportReader.get_report_data(time_period, emp_id)
        body = self._formatter.format_report(report_data) #invoke the method on the object we have instantiated
...
# could also inject through the method
# e.g. r.email_report_hours("a.nat@unsw.edu.au",12,1, PDFFormatter())
# high level method invokes method on resource, never has to instantiate a low-level module
